...
class FunnelPreTrainedModel(PreTrainedModel):
    ...
    def _get_name_mappings(cls, config: BertConfig) -> list[StateDictNameMapping]:
        ...
        for layer_index in range(config.num_decoder_layers):
                ...
                [
                    f"decoder.layers.{layer_index}.attention.r_r_bias",
                    f"decoder.layers.{layer_index}.attn.r_r_bias",
                ],
                [
                    f"decoder.layers.{layer_index}.attention.r_s_bias",
                    f"decoder.layers.{layer_index}.attn.r_s_bias",
                ],
                [
                    f"decoder.layers.{layer_index}.attention.seg_embed",
        ...
        # base-model prefix "FunnelModel"
        if "FunnelModel" not in config.architectures:
            for mapping in model_mappings:
                mapping[0] = "funnel." + mapping[0]
                mapping[1] = "funnel." + mapping[1]

        # downstream mappings
        if "FunnelForQuestionAnswering" in config.architectures:
